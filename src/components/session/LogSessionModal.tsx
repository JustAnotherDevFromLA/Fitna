...
export function LogSessionModal({ isOpen, onClose }: LogSessionModalProps) {
    ...
    useEffect(() => {
        ...
        if (isOpen && activeSession && !activeSession.endTime) {
    ...
    useEffect(() => {
        let interval: NodeJS.Timeout | undefined;
        if (isOpen && isTimerActive && restSeconds > 0) {
            interval = setInterval(() => {
        ...
        return () => clearInterval(interval);
    }, [isOpen, isTimerActive, restSeconds, activeSession, resumeSession]);
    ...
    return (
        <div style={{
            ...
            backgroundColor: '#0a0a0a', zIndex: 9999, overflowY: 'auto',
